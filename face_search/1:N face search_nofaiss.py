"""
AGPL-3.0 Licensed Single-File Flask Exact Face Search Service without FAISS.

This script provides a complete web and API service for exact 1:N face search.
It uses dlib / face_recognition to extract 128-dimensional face embeddings,
stores the startup-built face gallery in memory, and performs a full exhaustive
NumPy Euclidean L2-distance scan for every uploaded query image. It does not use
FAISS or any approximate nearest-neighbor index.
...
model, exact-search backend, and database folder path. If dlib can use CUDA, the service selects the CNN detector; otherwise it selects
HOG. Embeddings are always generated with dlib's large model. Runtime quality is
controlled by FACE_SEARCH_QUALITY=auto|balanced|high|max, where higher settings
increase upsampling and jitter count for more robust detection/encoding.

Run: python dlib_no_faiss_exact_l2_flask.py
...
NUMPY_DISTANCE_DTYPE = np.float64

VALID_QUALITY_LEVELS = {"auto", "balanced", "high", "max"}
QUALITY_PROFILES = {
    "balanced": {"upsample": 1, "jitters": 3},
    "high": {"upsample": 2, "jitters": 10},
    "max": {"upsample": 2, "jitters": 25},
}


def get_requested_quality() -> str:
    """Read the requested accuracy profile from FACE_SEARCH_QUALITY."""
    requested_quality = os.getenv("FACE_SEARCH_QUALITY", "auto").strip().lower()
    if requested_quality not in VALID_QUALITY_LEVELS:
        print(
            f"[face-search] invalid FACE_SEARCH_QUALITY={requested_quality!r}; using auto. "
            "Allowed values: auto, balanced, high, max.",
            flush=True,
        )
        return "auto"
...
def resolve_quality_settings(cuda_available: bool, requested_quality: str) -> Tuple[str, str, int, int]:
    """Return effective quality, detector, upsample count, and jitter count."""
    effective_quality = "high" if requested_quality == "auto" and cuda_available else requested_quality
    effective_quality = "balanced" if effective_quality == "auto" else effective_quality
    profile = QUALITY_PROFILES[effective_quality]
    detector_model = "cnn" if cuda_available else "hog"
    return effective_quality, detector_model, profile["upsample"], profile["jitters"]


FACE_SEARCH_QUALITY = get_requested_quality()
EFFECTIVE_FACE_SEARCH_QUALITY, FACE_DETECTION_MODEL, FACE_DETECTION_UPSAMPLE_TIMES, FACE_ENCODING_NUM_JITTERS = resolve_quality_settings(
...
def print_runtime_configuration() -> None:
    """Print dlib, CUDA, model, path, and exact-search settings for deployment diagnostics."""
    ...
    print(f"[face-search] requested quality: {FACE_SEARCH_QUALITY}", flush=True)
    print(f"[face-search] effective quality: {EFFECTIVE_FACE_SEARCH_QUALITY}", flush=True)
    ...
    print(f"[face-search] exact distance dtype: {NUMPY_DISTANCE_DTYPE.__name__}", flush=True)
    print(f"[face-search] database folder: {DATABASE_FOLDER}", flush=True)


@dataclass(frozen=True)
class IndexBuildReport:
    """Summary of one startup database scan and exact no-FAISS gallery build."""
...
class FaceSearchEngine:
    """Exact Euclidean L2-distance face search engine using raw dlib embeddings and NumPy."""

    def __init__(self) -> None:
        """Create locks and empty in-memory exact-search state."""
        ...
        self._latest_report = IndexBuildReport(
            ...
            index_type="numpy exact exhaustive 1:N scan",
            metric="euclidean_l2_distance over raw dlib embeddings",
            search_backend="numpy_float64_full_scan",
        )

    @staticmethod
    def _prepare_embedding(face_embedding: np.ndarray) -> np.ndarray:
        """Return one raw 128D dlib embedding as a contiguous float64 row for exact L2 search."""
        embedding = np.asarray(face_embedding, dtype=NUMPY_DISTANCE_DTYPE).reshape(1, EMBEDDING_DIMENSION)
            ...
            face_records.append(
                {
                    ...
                    "embedding": self._prepare_embedding(face_embedding),
                }
            )
        return face_records

    def rebuild_index(self, database_folder: Path = DATABASE_FOLDER) -> IndexBuildReport:
        """Re-scan gallery images, encode faces, and atomically replace the exact no-FAISS gallery."""
        with self._build_lock:
            ...
            embedding_matrix = np.empty((0, EMBEDDING_DIMENSION), dtype=NUMPY_DISTANCE_DTYPE)
            ...
            for image_number, image_path in enumerate(image_paths, start=1):
                ...
                try:
                    ...
                    for face_record in face_records:
                        ...
                        embeddings.append(face_record["embedding"])
            ...
            if embeddings:
                print(f"[face-search] preparing {len(embeddings)} face vector(s) for exact NumPy scan...", flush=True)
                embedding_matrix = np.vstack(embeddings)
                embedding_matrix = np.ascontiguousarray(embedding_matrix, dtype=NUMPY_DISTANCE_DTYPE)
                print(f"[face-search] exact gallery matrix ready. rows={embedding_matrix.shape[0]}", flush=True)
            else:
                print("[face-search] exact gallery matrix is empty because no valid face vector was produced.", flush=True)
    ...
    def get_snapshot(self) -> Tuple[Optional[np.ndarray], List[Dict[str, Any]], IndexBuildReport]:
        """Return a short-lived read snapshot for concurrent search requests."""
        with self._state_lock:
            return self._embedding_matrix, list(self._face_metadata), self._latest_report

    def get_stats(self) -> Dict[str, Any]:
        """Return current database, model, and no-FAISS status for UI/API display."""
        embedding_matrix, face_metadata, report = self.get_snapshot()
        ...
        return {
            **asdict(report),
            ...
            "vector_count": int(embedding_matrix.shape[0]) if embedding_matrix is not None else 0,
            "metadata_count": len(face_metadata),
            "requested_quality": FACE_SEARCH_QUALITY,
            "effective_quality": EFFECTIVE_FACE_SEARCH_QUALITY,
    ...
    def search_uploaded_image(self, image_path: Path, top_k: int, threshold: float) -> Dict[str, Any]:
        """Encode one query image and return exact Euclidean L2 matches sorted low to high."""
        embedding_matrix, face_metadata, report = self.get_snapshot()
        if embedding_matrix is None or embedding_matrix.shape[0] <= 0:
            raise RuntimeError(
                "Exact no-FAISS gallery is empty. Ask the server administrator to put gallery images into "
        ...
        query_embedding = self._prepare_embedding(query_face_encodings[0])
        indexed_face_count = int(embedding_matrix.shape[0])
        requested_top_k = int(top_k)
        search_count = indexed_face_count if requested_top_k <= 0 else min(requested_top_k, indexed_face_count)
        exact_search_started_at = time.perf_counter()
        differences = embedding_matrix - query_embedding
        all_squared_l2_distances = np.einsum("ij,ij->i", differences, differences, dtype=NUMPY_DISTANCE_DTYPE)
        vector_ids = np.arange(indexed_face_count)
        sorted_indices = np.lexsort((vector_ids, all_squared_l2_distances))
        result_indices = sorted_indices[:search_count]
        squared_l2_distances = all_squared_l2_distances[result_indices]
        exact_search_seconds = time.perf_counter() - exact_search_started_at
        print(f"[face-search] exact NumPy search finished: top_k={search_count}, seconds={exact_search_seconds:.4f}", flush=True)
        results: List[Dict[str, Any]] = []
        for rank_index, vector_index in enumerate(result_indices, start=1):
            if vector_index < 0:
                continue
            metadata = dict(face_metadata[int(vector_index)])
            squared_l2_distance = float(squared_l2_distances[rank_index - 1])
            l2_distance = float(np.sqrt(max(0.0, squared_l2_distance)))
            ...
            results.append(
                {
                    ...
                    "l2_distance": round(l2_distance, 8),
                    "squared_l2_distance": round(squared_l2_distance, 8),
                    "threshold": threshold,
                    "is_match": bool(l2_distance <= threshold),
                }
            )
        return {
            "success": True,
...
<main class="container py-4 py-lg-5">
    <section class="panel p-4 mb-4">
        <form id="searchForm" class="row g-3 align-items-end">
            ...
            <div class="col-sm-6 col-lg-2">
                <label class="form-label fw-bold">L2 距离阈值</label>
                <input class="form-control" type="number" name="threshold" min="0" step="0.01" value="0.60">
            </div>
            <div class="col-lg-2 d-grid">
                <button class="btn btn-theme btn-lg" type="submit">搜索</button>
            </div>
            <div class="col-12 small-muted">返回数量填 0 或留空表示返回全部结果；使用原始 dlib 128D embedding 的欧氏 L2 距离，距离越小越像；图片只用于本次查询，不会写入数据库。</div>
        </form>
    </section>
...
async function loadStats() {
    // Load current exact-search and gallery status from the backend.
    try {
        ...
        statsArea.innerHTML = `
            ...
            <div class="col-md-3"><div class="metric"><div class="small-muted">质量档位</div><div class="metric-value">${escapeHtml(data.effective_quality).toUpperCase()}</div></div></div>
            <div class="col-md-3"><div class="metric"><div class="small-muted">检测/编码</div><div class="metric-value">${escapeHtml(data.detection_model).toUpperCase()}/${escapeHtml(data.encoding_model).toUpperCase()}</div></div></div>`;
    } catch (error) {
    ...
    resultsArea.innerHTML = results.map(result => `<div class="col-md-6 col-xl-4"><div class="result-card">
        ...
        <div class="p-3">
            ...
            <div class="row g-2 small mb-3">
                <div class="col-6"><strong>L2 距离</strong><br>${result.l2_distance}</div>
                <div class="col-6"><strong>L2²</strong><br>${result.squared_l2_distance}</div>
                <div class="col-6"><strong>阈值</strong><br>${result.threshold}</div>
...
async function submitSearch(event) {
    // Upload one query image and render exact Euclidean L2-distance search results.
...
def parse_threshold(raw_value: Optional[str]) -> float:
    """Parse and validate the Euclidean L2-distance match threshold."""
    if raw_value is None or str(raw_value).strip() == "":
    ...
    if parsed_value < 0.0:
        raise ValueError("threshold must be greater than or equal to 0.0 for Euclidean L2 distance")
    return parsed_value
...
def api_stats():
    """Return current database and exact no-FAISS L2 status."""
...
def api_search():
    """Search one uploaded query image against the startup-built exact no-FAISS face database."""
...
if __name__ == "__main__":
    ...
    print("[face-search] starting service and building initial exact no-FAISS gallery...", flush=True)
